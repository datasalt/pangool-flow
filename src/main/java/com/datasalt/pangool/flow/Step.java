...
public abstract class Step implements Configurable, Tool, Serializable {
	...
	public int run(String[] args) throws Exception {
		...
		for(Input input : inputs) {
			if(!parsedInputs.containsKey(input.name)) {
				errors.add("Missing required input path: [" + input.name + "]. Please add it with -"
				    + input.name + " or --" + input.name);
			}
		}
		...
		for(Param param : parameters) {
			if(!parsedParameters.containsKey(param.getName())) {
				errors.add("Missing required parameter: [" + param + "]. Please add it with -D "
				    + hadoopConfName(param));
			}
		}
	 ...
	 */
	public abstract int run(Path outputPath, Map<String, Path> parsedInputs,
	 ...
	 */
	public int executeCoGrouper(TupleMRBuilder coGrouper) throws IOException, TupleMRException,
